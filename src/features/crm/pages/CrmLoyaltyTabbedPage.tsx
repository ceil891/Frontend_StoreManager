...
import { LoyaltyPointHistoryPage } from './LoyaltyPointHistoryPage';

const tabs = [
  { id: 'tiers', label: 'Hạng thành viên', icon: Award, permission: 'crm:tier:view' },
  { id: 'history', label: 'Lịch sử điểm thưởng', icon: History, permission: 'crm:loyalty-history:view' },
] as const;

type TabId = typeof tabs[number]['id'];

export function CrmLoyaltyTabbedPage() {
  const [searchParams, setSearchParams] = useSearchParams();
  const activeTabParam = searchParams.get('tab') as TabId | null;
  const activeTab = tabs.some(t => t.id === activeTabParam) ? (activeTabParam as TabId) : 'tiers';
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        {activeTab === 'tiers' && (
          <RoleGuard requiredPermission="crm:tier:view">
            <LoyaltyTiersPage />
          </RoleGuard>
        )}
        {activeTab === 'history' && (
          <RoleGuard requiredPermission="crm:loyalty-history:view">
...
export default CrmLoyaltyTabbedPage;
