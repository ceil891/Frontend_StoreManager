...
import { Download, AlertTriangle, TrendingDown, ArrowUpRight, ArrowDownRight, Archive, Package, Building2 } from 'lucide-react';
import { 
...
export function InventoryReportPage() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((p: any) => {
      if (p.branchId) return String(p.branchId) === String(storeSelect);
      return true;
    });
  }, [products, storeSelect]);
  ...
  const columns = useMemo<ColumnDef<LowStockItem>[]>(
    () => [
      ...
      },
      {
        ...
        cell: (info) => <span className="text-gray-500 dark:text-gray-400">{info.getValue() as number}</span>,
      },
      {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="flex items-center gap-2 px-4 py-2 bg-primary hover:bg-primary-hover text-white rounded-lg transition-colors text-sm font-medium shadow-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm flex flex-col">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">Tỷ trọng vốn lưu động phân bổ theo nhóm hàng</p>
          <div className="h-[300px] flex-1">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <RechartsTooltip 
                  formatter={(value) => `${(Number(value ?? 0) / 1000000).toFixed(1)} triệu đ`}
            ...
            {categoryStock.map((item) => (
              <div key={item.name} className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400">{(item.value / 1000000).toFixed(1)} triệu đ</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">Top sản phẩm tồn kho trong hệ thống</p>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={stockDistribution} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                ...
                <RechartsTooltip 
                  formatter={(value) => [`${Number(value ?? 0)} sản phẩm`, 'Tồn kho']}
                  cursor={{fill: 'transparent'}} 
                  contentStyle={{ backgroundColor: '#1F2937', borderColor: '#374151', color: '#fff', borderRadius: '12px' }} 
                />
                <Bar dataKey="stock" name="Tồn kho" fill="#0068FF" radius={[4, 4, 0, 0]} barSize={40}>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-bold text-gray-900 dark:text-white">Chi tiết tồn kho các mặt hàng ({lowStockItems.length} sản phẩm)</h3>
          </div>
        </div>
