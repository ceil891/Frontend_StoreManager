...
export function InventorySettingsPanel() {
  ...
  const handleSave = () => {
    if (tempThreshold < 0) {
      toast.error('Threshold không thể âm');
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
      ...
      <div className="space-y-4">
          ...
          <div className="flex items-end gap-3">
            <div className="flex-1">
              <input
                ...
                className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-white font-bold text-lg focus:ring-2 focus:ring-emerald-500"
              />
            ...
            <div className="text-right">
              <span className="text-2xl font-bold text-emerald-700">{tempThreshold}</span>
        ...
        <div className="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
          <p className="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider mb-3">Preview: Ví dụ hiển thị</p>
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className={`p-3 rounded-lg text-center ${tempThreshold >= 10 ? 'bg-red-50 border border-red-200' : 'bg-gray-100 border border-gray-200'}`}>
              ...
              <p className="text-xs text-gray-600 mt-1">
                {tempThreshold >= 10 ? '⚠️ Cảnh báo' : '✓ OK'}
              </p>
            </div>
            <div className={`p-3 rounded-lg text-center ${tempThreshold >= 20 ? 'bg-red-50 border border-red-200' : 'bg-gray-100 border border-gray-200'}`}>
              ...
              <p className="text-xs text-gray-600 mt-1">
                {tempThreshold >= 20 ? '⚠️ Cảnh báo' : '✓ OK'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
          ...
          <button
            onClick={handleSave}
            disabled={tempThreshold === inventorySettings.lowStockThreshold}
            className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg font-semibold text-sm disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            {isSaved ? (
              <>
      ...
      <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800 rounded-lg">
        <p className="text-sm text-blue-800 dark:text-blue-300">
          <strong>ℹ️ Lưu ý:</strong> Cài đặt này sẽ tác động tới toàn bộ hệ thống. Sản phẩm có tồn kho ≤ {tempThreshold} sẽ được cảnh báo màu đỏ trong danh mục hàng hóa.
        </p>
      </div>
    </div>
  );
