...
const tabGroups = [
  ...
  {
    groupName: 'Cấu trúc vị trí WMS',
    groupIcon: Layers,
    items: [
      { id: 'warehouses', label: 'Khu vực kho', icon: Warehouse, permission: 'inventory:storage-area:view' },
      { id: 'zones', label: 'Dãy kho', icon: Grid, permission: 'inventory:storage-area:view' },
      { id: 'bins', label: 'Ô / kệ kho', icon: Box, permission: 'inventory:storage-area:view' },
    ],
  },
] as const;
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-2 px-3.5 py-2 text-sm font-medium rounded-lg transition-all cursor-pointer whitespace-nowrap ${
                        isActive
                          ? 'bg-white dark:bg-gray-800 text-primary shadow-sm border border-gray-200 dark:border-gray-700 font-semibold'
