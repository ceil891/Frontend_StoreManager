...
export function WarehouseBinsPage() {
  ...
  const handleOpenEdit = (item: WarehouseBinRecord) => {
    ...
    setEditingItem({
      ...
      bay: bay,
      position: pos,
      binType: (item as any).binType || 'STORAGE',
      allowFood: (item as any).allowFood !== false,
      allowCosmetics: (item as any).allowCosmetics !== false,
      allowElectronics: !!(item as any).allowElectronics,
      allowChemicals: !!(item as any).allowChemicals,
      allowMixedSku: (item as any).allowMixedSku !== false,
      allowMixedLot: !!(item as any).allowMixedLot,
      statusConfig: (item as any).statusConfig || ((item.status as string) === 'LOCKED' ? 'LOCKED' : (item.status as string) === 'MAINTENANCE' ? 'MAINTENANCE' : (item.status as string) === 'INACTIVE' ? 'INACTIVE' : 'ACTIVE'),
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      binType: editingItem.binType || 'STORAGE',
      allowFood: editingItem.allowFood !== false,
      allowCosmetics: editingItem.allowCosmetics !== false,
      allowElectronics: !!editingItem.allowElectronics,
      allowChemicals: !!editingItem.allowChemicals,
      allowMixedSku: editingItem.allowMixedSku !== false,
      allowMixedLot: !!editingItem.allowMixedLot,
