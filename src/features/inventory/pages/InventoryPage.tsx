...
import { 
  Plus, Download, Search, Eye, Tag,
  MapPin, Image as ImageIcon, Edit, Trash2, AlertCircle, X, Upload,
  CircleDot, Package, Barcode
} from 'lucide-react';
...
export function InventoryPage() {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>, isGallery: boolean, idx?: number) => {
  ...
  const handleExportCsv = () => {
    ...
    exportToCsv('danh-muc-san-pham', data, [
      ...
      { header: 'Giá vốn', accessor: (row) => row.cost },
      { header: 'Tồn kho', accessor: (row) => row.stock },
  ...
  const columns = useMemo<ColumnDef<ProductInventory>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div>
            <span className={`font-bold ${row.original.onHand <= 5 ? 'text-red-600' : 'text-gray-900 dark:text-white'}`}>
              {row.original.onHand}
            </span>
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col gap-3 p-4 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
          ...
          <div className="flex flex-wrap items-center gap-3 pt-2 border-t border-gray-100 dark:border-gray-700/60">
            ...
            <div className="flex items-center gap-1.5 text-xs">
              <span className="text-gray-500 font-medium">Cập nhật từ:</span>
              <input 
                type="date"
                value={fromDate}
        ...
        {selectedProduct && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              ...
              <div className={`border p-4 rounded-xl ${selectedProduct.onHand <= 5 ? 'bg-red-50 border-red-100' : 'bg-gray-50 border-gray-200'}`}>
                <p className={`text-xs font-semibold uppercase ${selectedProduct.onHand <= 5 ? 'text-red-800' : 'text-gray-600'}`}>Tồn kho khả dụng</p>
                <p className={`text-2xl font-bold ${selectedProduct.onHand <= 5 ? 'text-red-700' : 'text-gray-900'}`}>{selectedProduct.onHand} {selectedProduct.unit}</p>
                {selectedProduct.onHand <= 5 && <p className="text-xs text-red-600 mt-1 flex items-center gap-1"><AlertCircle className="w-3 h-3" /> Cảnh báo sắp hết hàng</p>}
              </div>
            </div>
