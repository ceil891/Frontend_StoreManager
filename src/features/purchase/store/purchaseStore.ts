...
interface PurchaseState {
  ...
  addImportReceipt: (receipt: Omit<ImportReceiptItem, 'id'>) => void;
  updateImportReceipt: (id: string, data: Partial<ImportReceiptItem>) => void;
  deleteImportReceipt: (id: string) => void;
}
...
const MOCK_RECEIPTS: ImportReceiptItem[] = [
  { id: '1', grnNumber: 'GRN-2024-301', poNumber: 'PO-2024-801', supplierName: 'Global Tech Distribution', receivingStore: 'Main Flagship / HQ', receivedDate: '2024-05-17', totalItems: 45, acceptedItems: 45, rejectedItems: 0, totalValuation: 35000.00, status: 'INSPECTED_ACCEPTED', inspectedBy: 'Michael Chang', notes: 'All barcode scanners and receipt printers inspected. Perfect condition.' },
  { id: '2', grnNumber: 'GRN-2024-302', poNumber: 'PO-2024-789', supplierName: 'Apex Premium Packaging', receivingStore: 'Central Distribution Warehouse', receivedDate: '2024-05-16', totalItems: 5000, acceptedItems: 4850, rejectedItems: 150, totalValuation: 4100.00, status: 'PARTIAL_ACCEPTANCE', inspectedBy: 'David Ross', notes: '150 paper shopping bags suffered water damage during transit. Discrepancy logged for credit note.' },
  { id: '3', grnNumber: 'GRN-2024-303', poNumber: 'PO-2024-770', supplierName: 'Nordic Apparel Mills', receivingStore: 'Downtown Branch', receivedDate: '2024-05-15', totalItems: 350, acceptedItems: 0, rejectedItems: 0, totalValuation: 18500.00, status: 'PENDING_INSPECTION', inspectedBy: 'Sarah Jenkins', notes: 'Cargo unboxed on receiving dock. Awaiting QA verification.' },
];
...
export const usePurchaseStore = create<PurchaseState>()(
  persist(
    (set) => ({
      ...
      addImportReceipt: (receipt) =>
        set((state) => ({
          importReceipts: [{ id: Date.now().toString(), ...receipt }, ...state.importReceipts],
        })),
      updateImportReceipt: (id, data) =>
        set((state) => ({
          importReceipts: state.importReceipts.map((r) => (r.id === id ? { ...r, ...data } : r)),
        })),
      deleteImportReceipt: (id) =>
        set((state) => ({
          importReceipts: state.importReceipts.filter((r) => r.id !== id),
        })),
    }),
    {
